...
const findUserByIdentity = async (email?: string, phone?: string) => {
  ...
  return prisma.user.findFirst({ where: { OR: filters } });
};

const issueAccessToken = (user: { id: string; email: string | null; phone: string | null }) =>
  jwt.sign(
    { sub: user.id, email: user.email ?? undefined, phone: user.phone ?? undefined },
    env.jwtSecret,
...
const createAndSendOtp = async (userId: string, destination: string) => {
  ...
  const record = await prisma.otpCode.create({
    data: { userId, channel: 'email', destination, codeHash, expiresAt },
  });
  await sendOtpEmail(destination, code, env.otpExpiresMinutes);
  return { requestId: record.id, expiresAt };
};
...
authRouter.post('/register', async (req, res, next) => {
  try {
    ...
    if (email) {
      const { requestId, expiresAt } = await createAndSendOtp(user.id, email);
...
authRouter.post('/otp/request', async (req, res, next) => {
  try {
    ...
    if (!destination) {
      return res.status(400).json({ message: 'No email or phone on account' });
    }

    const { requestId, expiresAt } = await createAndSendOtp(user.id, destination);
    return res.status(200).json({ requestId, expiresAt });
  } catch (error) {
    return next(error as Error);
  }
});
...
authRouter.post('/otp/verify', async (req, res, next) => {
  try {
    const { email, phone, code, requestId } = req.body as {
      email?: string;
      phone?: string;
      code?: string;
      requestId?: string;
...
authRouter.post('/refresh', async (req, res, next) => {
  try {
    ...
    const { refreshToken: nextRefreshToken, refreshExpiresAt } = await issueRefreshToken(stored.userId);

    return res.status(200).json({ accessToken, refreshToken: nextRefreshToken, refreshExpiresAt });
  } catch (error) {
    return next(error as Error);
  }
});
